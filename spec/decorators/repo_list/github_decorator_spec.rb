...
RSpec.describe RepoList::GithubDecorator, type: :decorator do
  ...
  describe '#new' do
    ...
    it { expect(subject.current_page_items.map(&:keys)).to all(match_array(%i[name description url])) }
  end

  describe '#pagination_info' do
    subject { decorator_instance.pagination_info }

    it { expect(subject.keys).to match_array(%i[page per_page total_count]) }
